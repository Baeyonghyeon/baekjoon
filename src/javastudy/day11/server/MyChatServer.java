...
class MyChatServer {
    ...
    //접속 정보 추가
    protected void joinChat(ClientSession session) {
    ...
    protected void leaveChat(ClientSession session) {
    ...
    //개인 메시지 전달
    protected void sendToUser(ClientSession session, String message) {
    ...
    //개인 메시지 전달
    protected void sendToUser(ClientSession session, StringBuilder message) {
        if (clientOutMap.containsKey(session.id)) {
            try {
